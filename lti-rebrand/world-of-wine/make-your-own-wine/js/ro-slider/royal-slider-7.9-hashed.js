...
(function($) {
	...
	function RoyalSlider(element, options) {
		...
		this._setGrabCursor();
                ...
                if (this.settings.crossLinking && location.hash && parseInt(location.hash.slice(1)) <= this.numSlides ) {
                    this.currentSlideId = parseInt(location.hash.slice(1))-1;
                    this.offset = - (this.slideWidth*this.currentSlideId);
                    this._dragContainer.css ({left: this.offset})
                } else if (this.settings.startSlideIndex != 0 && this.settings.startSlideIndex <= this.numSlides) { 
                    this.currentSlideId = this.settings.startSlideIndex;
                    this.offset = - (this.slideWidth*this.currentSlideId);
                } else {
		...
		if(this.settings.controlNavEnabled) {
			...
			// added for crosslinking
			this.slides.each(function(index){    
                if(self.settings.controlNavThumbs) {                                
                    _navigation.append('<a href="#' + (index + 1) + '" class="royalThumb transition" style="background-image:url('+ $(this).attr("data-thumb") +')">' + (index + 1) + '');
                } else {                    
                    _navigation.append('<a href="#' + (index + 1) + '">' + (index + 1) + '</a>');    
                }    
                cSlideId++;    
            });   	
	...
	RoyalSlider.prototype = {
			...
			prev:function() {
                ...
                if (this.settings.crossLinking) {
                    window.location.hash = "#" + (this.currentSlideId+1)
                }
            },
			...
			_startSlider:function() {
				...
				if(this.settings.controlNavEnabled) {
					this.navItems.bind("click", function(e){ 
						if (!self.settings.crossLinking) { e.preventDefault(); }   //crosslinking edit
	...
	$.fn.royalSlider.defaults = {    
			...
			crossLinking:true,                      // Allows hashtags
